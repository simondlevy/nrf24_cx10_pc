...
static uint8_t NRF24L01_WriteReg(uint8_t address, uint8_t data)
{
    CS_off;
...
static void NRF24L01_WriteRegisterMulti(uint8_t address, const uint8_t data[], uint8_t len)
{
    delayMicroseconds(5);
    CS_off;
...
static void NRF24L01_Initialize()
{
    rf_setup = 0x0F;
}
...
static uint8_t NRF24L01_FlushTx()
{
    return Strobe(FLUSH_TX);
}

static uint8_t NRF24L01_FlushRx()
{
    return Strobe(FLUSH_RX);
}

static uint8_t NRF24L01_WritePayload(uint8_t *data, uint8_t length)
{
    CE_off;
    CS_off;
...
static uint8_t NRF24L01_ReadPayload(uint8_t *data, uint8_t length)
{
    uint8_t i;
    CS_off;
...
static void NRF24L01_SetTxRxMode(enum TXRX_State mode)
{
    if(mode == TX_EN) {
        CE_off;
...
static uint8_t NRF24L01_SetPower(enum TX_Power power)
{
    rf_setup = (rf_setup & 0xF9) | ((power & 0x03) << 1);
    return NRF24L01_WriteReg(NRF24L01_06_RF_SETUP, rf_setup);
}

static uint8_t NRF24L01_SetBitrate(uint8_t bitrate)
{
